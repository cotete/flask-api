...
@app.route('/api/metas/<string:email>', methods=['POST'])
def create_meta(email):
    data = request.get_json()
    metas.insert_one(data)
    result = metas.find_one(data)
    result['_id'] = str(result['_id'])
    return jsonify(result), 201

@app.route('/api/metas/<string:email>', methods=['GET'])
def get_metas(email):
    result = list(metas.find({'email': email}))
    for meta in result:
        meta['_id'] = str(meta['_id'])
    return jsonify(result), 200

@app.route('/api/metas/<string:id>', methods=['DELETE'])
def delete_meta(id):
    metas.delete_one({'_id': id})
